...
uses
  Classes, Windows, Graphics, Messages, Controls, Forms, SysUtils, System.Types;

type
  TArrowPos = (NE, NW, SE, SW);
  TJcHint = class(TComponent)
  private
    FHintFont: TFont;
  ...
  published
    ...
    property ColorArrowBackground: TColor index 2 read FArrowBackgroundColor write SetColors default$00E3CAAE;
  ...
  TJcHintWindow = class(THintWindow)
  private
    FArrowPos: TArrowPos; 
    FArrowPoint: TPoint;
    FHint: TJcHint;
    function FindFlatHint: TJcHint;
  protected
...
procedure Register;
begin
  RegisterComponents('Jc Components', [TJcHint]);
end;

constructor TJcHint.Create (AOwner: TComponent);
begin
  inherited Create(AOwner);

  if not (csDesigning in ComponentState) then
  begin
    HintWindowClass := TJcHintWindow;
...
destructor TJcHint.Destroy;
begin
  FHintFont.Free;
  inherited Destroy;
end;

procedure TJcHint.SetColors (Index: Integer; Value: TColor);
begin
  case Index of
...
procedure TJcHint.SetHintFont (Value: TFont);
begin
  FHintFont.Assign(Value);
end;

procedure TJcHint.GetHintInfo (var HintStr: string; var CanShow: Boolean; var HintInfo: THintInfo);
begin
...
function TJcHintWindow.FindFlatHint: TJcHint;
var
...
begin
  ...
  with Application.MainForm do
    for currentComponent := 0 to ComponentCount - 1 do
      if Components[currentComponent] is TJcHint then
      begin
        Result := TJcHint(Components[currentComponent]);
        Break;
      end;
end;

procedure TJcHintWindow.CreateParams (var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.Style := Params.Style - WS_BORDER;
end;

procedure TJcHintWindow.Paint;
var
...
procedure TJcHintWindow.ActivateHint (HintRect: TRect; const AHint: string);
var
  curWidth: Byte;
  Pnt: TPoint;
